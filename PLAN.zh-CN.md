...
- `material_inventory_items`：板材物料库存项，覆盖整料 / 余料，记录宽、长、厚度、材质、数量、来源、位置、状态、可复用性。当前字段口径来自 `Template.xlsx` 的板材类物料；管材、型材等库存字段后续单独规划。
...
目标：建立板材物料库存的核心表和 API。第一版覆盖整料和余料，管材 / 型材库存暂缓。

任务：

- [x] 实现材质 / 厚度 / 规格基础数据。
- [x] 实现板材物料库存 CRUD，覆盖整料和余料。
- [x] 实现状态流转：`available`、`reserved`、`consumed`、`scrapped`、`voided`。
- [x] 支持按材质、厚度、宽长、状态、位置、库存类型查询库存项。
